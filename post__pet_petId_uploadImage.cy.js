 import Ajv from "ajv";
							...
							let requestInfo = JSON.parse(JSON.stringify({"url":"/pet/{petId}/uploadImage","method":"POST"}));
							...
							describe('uploads an image', () => {
							  // Please be informed that the API in this file has been deprecated and will no longer be supported.
								it('successful operation' ,()=>{
																	cy.fixture("200_application_json_multipart_form-data_post__pet_petId_uploadImage").then((fixtureResponse) => {
																	cy.fixture('**filePath**').then((fileContent) => {
																	const blob = new Blob([fileContent],{type : requestInfo.headers["Content-Type"]});
			...
			for (const key in pathParams) {
				if (pathParams.hasOwnProperty(key)) {
					...
					requestInfo.url = requestInfo.url.replace(new RegExp(placeholder, 'g'), pathParams[key]);
																		...
																		requestInfo.cookies = fixtureResponse.cookie ? fixtureResponse.cookie : "";
